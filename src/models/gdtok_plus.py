...
class GDTokPlus(nn.Module):
  
  def __init__(self, config):
    super().__init__()
    
    self.config = config
    self.name = 'GDTokPlus_' + config.get_extension()
    ...
    N_reg = 1.0 / torch.arange(1, config.context_size, device=self.wte.weight.device).unsqueeze(1).float() # 1/N term
  ...
  def forward(self, x, targets=None):
    ...
    # Krn
    x_i = p + e # Add positional embeddings to token embeddings (like GPT)
    x_j = p + e
