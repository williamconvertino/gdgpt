...
class GDTokConfig:
  ...
  def __post_init__(self):
    ...
    assert self.tok_alignment in ['n', 'n+1'], 'Invalid token alignment, must be "n" or "n+1"'

class GDTok(nn.Module):
  
  def __init__(self, config):
    super().__init__()
    
    self.config = config
    self.name = 'GDTok_' + config.get_extension()
  ...
  def forward(self, x, targets=None):
    ...
    # Krn
    if self.config.tok_alignment == 'n':
      x_i = p[:, :-1, :] + e
      x_j = p[:, :-1, :] + e      
    elif self.config.tok_alignment == 'n+1':
      x_i = p[:, :-1, :] + e
      x_j = p[:, 1:, :] + e
