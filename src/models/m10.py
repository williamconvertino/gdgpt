...
class Attention(nn.Module):
  ...
  def forward(self, e, p):
    ...
    p_q = p[:, 1:, :].repeat(1, 1, self.config.n_head).view(B, S, self.config.n_head, self.config.d_embed).transpose(1, 2)
    p_k = p[:, :-1, :].repeat(1, 1, self.config.n_head).view(B, S, self.config.n_head, self.config.d_embed).transpose(1, 2)
    ...
    Q = p_q @ torch.diag_embed(self.W_q)
    K = p_k @ torch.diag_embed(self.W_k)
    V = e - E_wte
...
class M10(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.config = config
    self.name = f'M10_{config.get_extension()}'
